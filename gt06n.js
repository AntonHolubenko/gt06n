module.exports.gt06 = (function(){
    ...
    var parseDatetime = function (data) {
        ...
        var y = parseInt(parse.year, 16);
        var m = parseInt(parse.month, 16);
        var d = parseInt(parse.day, 16);
        var h = parseInt(parse.hour, 16);
        var i = parseInt(parse.minute, 16);
        var s = parseInt(parse.second, 16);
        var datetime = [y,m,d].join('-') + ' ' +  [h,i,s].join(':');
        return datetime;
    };

    var hex2bin = function (hex) {
        var bin = (parseInt(hex, 16)).toString(2);
        var hexLen = hex.length/2*8;
        for( var i =0; i < 8; i++){
    ...
    var parseLocation = function(data){
        ...
        var courseBin = hex2bin(datasheet.course);
        var parsed = {
    ...
    var parseStatusPackage = function(courseBin){
        return {
            'real_time_gps': courseBin.substr(2,1),
            'gps_positioned': courseBin.substr(3,1),
            'east_longitude': courseBin.substr(4,1),
    ...
    var parseStatus = function (data) {
        var statusBin = hex2bin(data.substr(8,6));
        var parsed = parseStatusPackage(statusBin);
        return parsed;
    ...
    var selectEvent = function (data) {
        var event = data.substr(6,2);
        return event;
    };
